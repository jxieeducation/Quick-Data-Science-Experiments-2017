...
def prepare_dataset(questions, answers):
    ...
    make_dir("processed_cornell_movie_dialogs_data")

    # random convos to create the test set
    test_ids = random.sample([i for i in range(len(questions))], 25000)

    filenames = ['train.enc', 'train.dec', 'test.enc', 'test.dec']
    files = []
    for filename in filenames:
        files.append(open(os.path.join("processed_cornell_movie_dialogs_data", filename), 'wb'))
...
def load_vocab(vocab_fname):
    print("load vocab ...")
    with open(os.path.join("processed_cornell_movie_dialogs_data", vocab_fname), 'rb') as f:
...
def build_vocab(in_fname, out_fname, normalize_digits=True):
    ...
    in_path = os.path.join("processed_cornell_movie_dialogs_data", in_fname)
    out_path = os.path.join("processed_cornell_movie_dialogs_data", out_fname)
    ...
    dest_path = os.path.join("processed_cornell_movie_dialogs_data", 'vocab')
...
def token2id(data, mode):
    ...
    in_file = open(os.path.join("processed_cornell_movie_dialogs_data", in_path), 'rb')
    out_file = open(os.path.join("processed_cornell_movie_dialogs_data", out_path), 'wb')
...
def make_train_and_test_set():
    print("make Training data and Test data ...")
    ...
    if len(train_X) == len(train_y) and len(test_X) == len(test_y):
        print("train data count : %d" % len(train_X))
        print("test data count : %d" % len(test_X))
        return train_X, test_X, train_y, test_y
    else:
        train_count = min(len(train_X), len(train_y))
        test_count = min(len(test_X), len(test_y))

        print("train data count : %d" % train_count)
        print("test data count : %d" % test_count)

        return train_X[:train_count], test_X[:test_count], train_y[:train_count], test_y[:test_count]

def load_data(enc_fname, dec_fname):
    enc_input_data = open(os.path.join("processed_cornell_movie_dialogs_data", enc_fname), 'r')
    dec_input_data = open(os.path.join("processed_cornell_movie_dialogs_data", dec_fname), 'r')
    ...
    for e_line, d_line in zip(enc_input_data.readlines(), dec_input_data.readlines()):
        ...
        if len(e_ids) <= 200 and len(d_ids) < 200:
            enc_data.append(_pad_input(e_ids, 200))
            dec_data.append(_pad_input(d_ids, 200))
